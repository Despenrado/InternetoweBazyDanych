...
const Form = ({submitForm}) => {
   ...
   const Routes=[];
    ...
    fetchedData.map((data,index) => {
        Routes.push({value:data.TRASA_ID,label:`ID: ${data.ID_TRASA} City: ${data.MIASTO_TRASA} Start: ${data.POCZATEK_TRASA} Finish: ${data.KONIEC_TRASA} Km: ${data.DLUGOSC_TRASA}`, color:'#00B8D9'})
    })    
    ...
    return(
        <div className={"form__container"}>
            ...
            <form name={"addRunForm"} className={"form"} onClick={() => cleanErrors(errors, updateErrors)}>
                ...
                <label className={"form__label"} htmlFor={"route"}>Choose ID route</label>

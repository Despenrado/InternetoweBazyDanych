...
async function closeRun(e, Fromdata) {
  ...
  await submitForm(
    `http://0.0.0.0:3102/organizer/${Fromdata.ID_BIEG}/finish`,
    data
  );

  //Skasowanie biegu
  await deleteData(`http://0.0.0.0:3102/organizer/${Fromdata.ID_BIEG}/delete`);
}
const validateForm = async (e, changeState) => {
  ...
  if (errors.length) changeState(errors);
    ...
    await updateUser(
      `http://0.0.0.0:3102/organizer/${id}/edit`,
      data,
      changeState
    );
    form["name"].value = "";
...
const signRunner = async (e, updateState) => {
  ...
  const res = await fetch(`http://0.0.0.0:3102/${id}/runner`, {
    method: "POST",
    mode: "cors",
    credentials: "include",
  });
      ...
      {!visibility ? (
        <div className={"run__left"}>
          ...
          {localStorage.getItem("type") === "biegacz" && (
            <button onClick={(e) => signRunner(e, updateText)}>
              {"Join to run!"}
            </button>
          )}
          {/* {
          ...
          {localStorage.getItem("type") === "organizator" &&
            window.location.href === "http://127.0.0.1:3000/organizer/runs" && (
              <a href={`/organizer/runners/${data.ID_BIEG}`}>Show runners</a>
            )}
          {localStorage.getItem("type") === "organizator" &&
            window.location.href === "http://127.0.0.1:3000/organizer/runs" && (
              <button onClick={(e) => showForm(!visibility)}>
                {"Edytuj bieg"}
              </button>
            )}
          {localStorage.getItem("type") === "organizator" &&
            window.location.href === "http://127.0.0.1:3000/organizer/runs" && (
              <a
                href={`/organizer/Finishrun/${data.ID_BIEG}`}
                onClick={(e) => closeRun(e, data)}
              >
                Close run
              </a>
            )}

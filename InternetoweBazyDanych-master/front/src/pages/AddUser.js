...
// Variables
const url = "http://0.0.0.0:3102/admin/add";
...
const validateForm = async (e, changeState) => {
  ...
  if (
    ...
    form["type"].value !== "biegacz" &&
    form["type"].value !== "organizator"
  )
    errors.push({
      message:
        "Incorrect user type. Enable values are: biegacz, organizator, admin!",
      success: false,
    });
  if (errors.length) changeState(errors);
    ...
    data.birth_date = form["Date"].valuel;
